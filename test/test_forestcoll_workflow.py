...
for i, layer in enumerate(topology.layers):
    ...
    for node in layer:
        sibling_ids = sorted( node.siblings)
...
for layer in topology.layers:
    for node in layer:
        for neighbor_node_id in node.siblings:
            if node.node_id not in topology.nodes[neighbor_node_id].siblings:
                print(f"Error: Node {node.node_id} connects to {neighbor_node_id}, but {neighbor_node_id} does not connect back.")
